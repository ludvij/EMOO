...
{\
	do { \
		std::streambuf *old_cout = std::cout.rdbuf(); std::stringstream ss_cout; std::cout.rdbuf(ss_cout.rdbuf()); \
		std::streambuf *old_cerr = std::cerr.rdbuf(); std::stringstream ss_cerr; std::cerr.rdbuf(ss_cerr.rdbuf()); \
		std::streambuf *old_clog = std::clog.rdbuf(); std::stringstream ss_clog; std::clog.rdbuf(ss_clog.rdbuf()); \
		code \
		std::cout.rdbuf(old_cout); \
		std::cerr.rdbuf(old_cerr); \
		std::cerr.rdbuf(old_clog); \
	} while (0);\
}

#define assert_no_print(assert_type, v1, v2) NO_PRINT(Lud::assert_##assert_type(v2, v2))
