...
SDL3Input::~SDL3Input()
{
	if (m_controller)
	{
		std::println("Removed controller ID: [{:d}], name: [{:s}]", SDL_GetGamepadInstanceID(m_controller), SDL_GetGamepadName(m_controller));
		SDL_CloseGamepad(m_controller);
		m_controller = nullptr;
	}
}
bool SDL3Input::GetButton(Button b) const
{
	const auto p = SDL_GetGamepadButton(m_controller, get_sdl_equivalent(b));
	return p;
}

bool SDL3Input::GetKey(Key k) const
{
	return m_keyboard_state[get_sdl_equivalent(k)] == 1;
}

void SDL3Input::add_controller()
{
...
void SDL3Input::remove_controller()
{
	if (m_controller == nullptr)
	{
		return;
...
void SDL3Input::platform_override()
{
	int count;
...
void SDL3Input::ProcessEvents(void* event)
{
