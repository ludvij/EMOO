#include "pch.hpp"

#include "TestConfig.hpp"

class TestBranch : public TestFixture
{
};

TEST_F(TestBranch, GENERAL_BRANCH_PAGECROSS)
{
	asse.Assemble(R"(
		bcc -13
	)");

	clearCycles();
	// pc is 2, and the offset is
	// -13 so offset + pc = -11
	// pc is u16 so it wraps to $0000 - 11 = $fff5
	ASSERT_EQ(bus.GetCpu().PC(), 0xfff5);
	// page crossing
	ASSERT_EQ(bus.GetCpu().GetCycles(), 3);
}

TEST_F(TestBranch, BCC_BRANCH)
{
	asse.Assemble(R"(
		bcc 2
	)");
	clearCycles();

	ASSERT_EQ(bus.GetCpu().PC(), 4);
	// no page crossing
	ASSERT_EQ(bus.GetCpu().GetCycles(), 2);
}

TEST_F(TestBranch, BCC_NOBRANCH)
{
	bus.GetCpu().SetP(Emu::P_C_FLAG);
	asse.Assemble(R"(
		bcc 2
	)");

	clearCycles();

	// no branching
	ASSERT_EQ(bus.GetCpu().PC(), 2);
	ASSERT_EQ(bus.GetCpu().GetCycles(), 1);
}

TEST_F(TestBranch, BCS_BRANCH)
{
	bus.GetCpu().SetP(Emu::P_C_FLAG);
	asse.Assemble(R"(
		bcs 2
	)");

	clearCycles();

	ASSERT_EQ(bus.GetCpu().PC(), 4);
	// no page crossing
	ASSERT_EQ(bus.GetCpu().GetCycles(), 2);
}

TEST_F(TestBranch, BCS_NOBRANCH)
{
	asse.Assemble(R"(
			bcs 2
		)");

	clearCycles();

	// no branching
	ASSERT_EQ(bus.GetCpu().PC(), 2);
	ASSERT_EQ(bus.GetCpu().GetCycles(), 1);
}

TEST_F(TestBranch, BEQ_BRANCH)
{
	bus.GetCpu().SetP(Emu::P_Z_FLAG);
	asse.Assemble(R"(
			beq 2
		)");

	clearCycles();

	ASSERT_EQ(bus.GetCpu().PC(), 4);
	// no page crossing
	ASSERT_EQ(bus.GetCpu().GetCycles(), 2);
}

TEST_F(TestBranch, BEQ_NOBRANCH)
{
	asse.Assemble(R"(
			beq 2
		)");

	clearCycles();

	// no branching
	ASSERT_EQ(bus.GetCpu().PC(), 2);
	ASSERT_EQ(bus.GetCpu().GetCycles(), 1);
}

TEST_F(TestBranch, BMI_BRANCH)
{
	bus.GetCpu().SetP(Emu::P_N_FLAG);
	asse.Assemble(R"(
			bmi 2
		)");

	clearCycles();

	ASSERT_EQ(bus.GetCpu().PC(), 4);
	// no page crossing
	ASSERT_EQ(bus.GetCpu().GetCycles(), 2);
}

TEST_F(TestBranch, BMI_NOBRANCH)
{
	asse.Assemble(R"(
			bmi 2
		)");

	clearCycles();

	// no branching
	ASSERT_EQ(bus.GetCpu().PC(), 2);
	ASSERT_EQ(bus.GetCpu().GetCycles(), 1);
}

TEST_F(TestBranch, BNE_BRANCH)
{
	asse.Assemble(R"(
			bne 2
		)");

	clearCycles();

	ASSERT_EQ(bus.GetCpu().PC(), 4);
	// no page crossing
	ASSERT_EQ(bus.GetCpu().GetCycles(), 2);
}

TEST_F(TestBranch, BNE_NOBRANCH)
{
	bus.GetCpu().SetP(Emu::P_Z_FLAG);
	asse.Assemble(R"(
			bne 2
		)");

	clearCycles();

	// no branching
	ASSERT_EQ(bus.GetCpu().PC(), 2);
	ASSERT_EQ(bus.GetCpu().GetCycles(), 1);
}

TEST_F(TestBranch, BPL_BRANCH)
{
	asse.Assemble(R"(
			bpl 2
		)");

	clearCycles();

	ASSERT_EQ(bus.GetCpu().PC(), 4);
	// no page crossing
	ASSERT_EQ(bus.GetCpu().GetCycles(), 2);
}

TEST_F(TestBranch, BPL_NOBRANCH)
{
	bus.GetCpu().SetP(Emu::P_N_FLAG);
	asse.Assemble(R"(
			bpl 2
		)");
	clearCycles();

	// no branching
	ASSERT_EQ(bus.GetCpu().PC(), 2);
	ASSERT_EQ(bus.GetCpu().GetCycles(), 1);
}

TEST_F(TestBranch, BVC_BRANCH)
{
	asse.Assemble(R"(
			bvc 2
		)");

	clearCycles();

	ASSERT_EQ(bus.GetCpu().PC(), 4);
	// no page crossing
	ASSERT_EQ(bus.GetCpu().GetCycles(), 2);
}

TEST_F(TestBranch, BVC_NOBRANCH)
{
	bus.GetCpu().SetP(Emu::P_V_FLAG);
	asse.Assemble(R"(
			bvc 2
		)");

	clearCycles();

	// no branching
	ASSERT_EQ(bus.GetCpu().PC(), 2);
	ASSERT_EQ(bus.GetCpu().GetCycles(), 1);
}

TEST_F(TestBranch, BVS_BRANCH)
{
	bus.GetCpu().SetP(Emu::P_V_FLAG);
	asse.Assemble(R"(
			bvs 2
		)");

	clearCycles();

	ASSERT_EQ(bus.GetCpu().PC(), 4);
	// no page crossing
	ASSERT_EQ(bus.GetCpu().GetCycles(), 2);
}

TEST_F(TestBranch, BVS_NOBRANCH)
{
	asse.Assemble(R"(
			bvs 2
		)");

	clearCycles();

	// no branching
	ASSERT_EQ(bus.GetCpu().PC(), 2);
	ASSERT_EQ(bus.GetCpu().GetCycles(), 1);
}
